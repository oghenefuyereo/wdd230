function getWeather2(latitude2, longitude2) {
    ...
    // Fetch weather data2 from API using latitude2 and longitude2
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude2}&lon=${longitude2}&appid=${apiKey2}&units=metric`)
        ...
        .catch(error2 => {
            ...
            alert('Failed to fetch weather data2. Please try again later.');
        });
}
...
function displayWeather2(data2) {
    ...
    // Find the paragraph element within the section
    const paragraph2 = document.querySelector('.weather p');

    // Update the content of the paragraph2 with the weather information
...
// Call getLocation2() when the page loads to fetch weather for the user's location
